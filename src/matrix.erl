...
matrix(Width, Height, GenFun) ->
  B =
  lists:sort(lists:flatten(lists:duplicate(Width, lists:seq(0, Height - 1)))),
...
from_list_test() ->
  ...
  ?assertEqual(
    #matrix{width = 3, height = 3, payload = <<1, 2, 3, 4, 5, 6, 7, 8, 9>>},
    from_list(List)).

matrix_test() ->
  ?assertEqual(
    #matrix{width = 3, height = 3, payload = <<1, 2, 3, 2, 4, 6, 3, 6, 9>>},
